...
void CPolygon1::Init()
{
	...
	m_Shader->Init( "shader2DTextureVS.cso", "shader2DTexture1PS.cso" );
	

}


void CPolygon1::Uninit()
{
	m_Shader->Uninit();
	delete m_Shader;
...
void CPolygon1::Update(float dt)
{

	iTime += dt;

	m_Shader->SetTime(iTime);
...
void CPolygon1::Draw()
{
	...
	m_Shader->SetResolutin(XMFLOAT3(SCREEN_WIDTH, SCREEN_HEIGHT, 0));
	m_Shader->SetiMouse({ CInput::GetMousePos().x,CInput::GetMousePos().y,
		(float)CInput::IsMouseClick(),(float)CInput::IsMouseClick() });

	CCamera* camera = CCamera::GetInstance();
	m_Shader->SetCameraPosition(&camera->GetPosition());
	m_Shader->SetCameraDirection(&camera->GetCameraDirection());
